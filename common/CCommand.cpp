...
bool CCommand::Initialize( const int iArgc, void* pData, const GetArg getArg )
{
	...
	//Make sure the arguments string is returned without a leading whitespace.
	if( m_iArgc > 1 )
		m_uiCommandNameLength += 1;
...
bool CCommand::Initialize( const char* pszCommand )
{
	...
	while( 1 )
	{
		...
		pszCommand = COM_Parse( pszCommand, szBuffer, sizeof( szBuffer ) );
		if( !pszCommand ) break;
