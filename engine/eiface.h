...
struct enginefuncs_t
{
	...
	*	@param pszModelName Name of the model whose index is to be returned.
	*	@return Index of the model.
	*/
	int			(*pfnModelIndex)			( const char* pszModelName );
	...
	*	@param pszLevelName Name of the level to change to.
	*	@param pszLandmarkName Name of the landmark to use. If null, no landmark is used.
	*/
	void		(*pfnChangeLevel)			( const char* pszLevelName, const char* pszLandmarkName );

	/**
	*	Does nothing useful. Will trigger a host error if the given entity is not a client.
	*/
	void		(*pfnGetSpawnParms)			( edict_t* pClient );

	/**
	*	Does nothing useful. Will trigger a host error if the given entity is not a client.
	*	Will trigger a sys error if the given entity is invalid.
	*/
	void		(*pfnSaveSpawnParms)		( edict_t * pClient );
	...
	*	@param pEntity Entity whose yaw is so be changed.
	*/
	void		(*pfnChangeYaw)				( edict_t* pEntity );
	...
	*	@param pEntity Entity whose pitch is to be changed.
	*/
	void		(*pfnChangePitch)			( edict_t* pEntity );
	...
	*	@param iszClassName Name of the class to instantiate.
	*	@return Edict of the entity that was instantiated, or null if no such entity exists.
	*/
	edict_t*	(*pfnCreateNamedEntity)		( string_t iszClassName );
	...
	*	@param pEntity Entity to make static.
	*/
	void		(*pfnMakeStatic)			( edict_t* pEntity );
	...
	*/
	int			(*pfnEntIsOnFloor)			( edict_t* pEntity );
	...
	*/
	int			(*pfnWalkMove)				( edict_t* pEntity, float yaw, float dist, int iMode );
	...
	*/
	void		(*pfnEmitSound)				( edict_t* pEntity, int channel, const char* pszSample, float volume, float attenuation, int fFlags, int pitch );
	...
	*	@param ptr TraceResult instance.
	*/
	void		(*pfnTraceToss)				( edict_t* pEntity, edict_t* pentToIgnore, TraceResult* ptr );
	...
	*/
	void		(*pfnServerCommand)			( const char* pszCommand );
	...
	*/
	void		(*pfnClientCommand)			( edict_t* pEdict, const char* pszFormat, ... );
	...
	*/
	int			(*pfnDecalIndex)			( const char* pszName );
	...
	*	@param iMsgType Message type. Used to be called msg_dest. @see NetMessageType
	*	@param iMsgID Message ID. Used to be called msg_type.
	...
	*/
	void		(*pfnMessageBegin)			( int iMsgType, int iMsgID, const float* pOrigin, edict_t* pEdict );
	...
	*	If no message had been started, triggers a sys error.
	*/
	void		(*pfnWriteString)			( const char* pszString );
	...
	*/
	float		(*pfnCVarGetFloat)			( const char* pszVarName );
	...
	*	@param pszVarName Name of the cvar whose value to set.
	*	@param flValue Value to set.
	*/
	void		(*pfnCVarSetFloat)			( const char* pszVarName, float flValue );

	/**
	*	Sets the value of a cvar as a string.
	*	@param pszVarName Name of the cvar whose value to set.
	*	@param pszValue Value to set. The string is copied.
	*/
	void		(*pfnCVarSetString)			( const char* pszVarName, const char* pszValue );
	...
	*	@param ... Format arguments.
	*/
	void		(*pfnAlertMessage)			( ALERT_TYPE aType, const char* pszFormat, ... );

	/**
	*	Obsolete. Will print a message to the server console using pfnAlertMessage indicating if it's being used.
	*/
	void		(*pfnEngineFprintf)			( void* pFile, const char* pszFormat, ... );
	...
	*	@param pszValue String to allocate.
	*	@param Index assigned to the string.
	*/
	string_t	(*pfnAllocString)			( const char* pszValue );
	...
	*/
	func_t		(*pfnFunctionFromName)		( const char* pszName );
	...
	*	@param pszMessage Message to send.
	*/
	void		(*pfnClientPrintf)			( edict_t* pEdict, PRINT_TYPE pType, const char* pszMessage ); // JOHN: engine callbacks so game DLL can print messages to individual clients
	...
	*	@param pBuffer Buffer to process.
	*	@param len Number of bytes in the buffer to process.
	*/
	void        (*pfnCRC32_ProcessBuffer)   ( CRC32_t* pulCRC, void* pBuffer, int len );
	...
	*	@param pszFilename Name of the file. Path starts in the game directory.
	...
	*/
	byte *      (*pfnLoadFileForMe)         ( const char* pszFilename, int* pLength );
	...
	*/
	...
	/**
	*	Compares file times.
	*	@param pszFilename1 First file to compare.
	*	@param pszFilename2 Second file to compare.
	*	@param piCompare	If both files are equal, 0.
	*						If the first file is older, -1.
	*						If the second file is older, 1.
	*	@return true if both filenames are non-null, false otherwise.
	*/
	int 		(*pfnCompareFileTime)       ( const char* pszFilename1, const char* pszFilename2, int* piCompare );
	
	/**
	*	Gets the game directory name.
	*	@param pszGetGameDir Buffer to store the game directory name in. Must be at least MAX_PATH bytes large.
	*/
	void        (*pfnGetGameDir)            ( char* pszGetGameDir );
	...
	*	@param flNewMaxspeed Maximum speed value.
	*/
	void        (*pfnSetClientMaxspeed)     ( const edict_t* pEdict, float flNewMaxspeed );

	/**
	*	Creates a fake client (bot).
	*	@param pszNetName Name of the client to show.
	*	@return The fake client, or null if it can't be created.
	*/
	edict_t *	(*pfnCreateFakeClient)		( const char* pszNetName );
	...
	*/
	char*		(*pfnGetInfoKeyBuffer)		( edict_t* pClient );

	/**
	*	Gets the value of the given key from the given buffer.
	*	@param pszInfoBuffer Buffer to query.
	*	@param pszKey Key whose value to retrieve.
	*	@return The requested value, or an empty string.
	*/
	char*		(*pfnInfoKeyValue)			( const char* pszInfoBuffer, const char* pszKey );
	...
	*	@param pszFilename Name of the map to check.
	*	@return true if the map is valid, false otherwise.
	*/
	int			(*pfnIsMapValid)			( const char* pszFilename );
	...
	*	@param pEntity Entity that is playing the sound.
	*	@param channel Channel to play the sound on.
	...
	*/
	...
	/**
	*	@param pszVarName Name of the cvar to retrieve.
	*	@return Cvar pointer, or null if the cvar doesn't exist.
	*/
	cvar_t		*(*pfnCVarGetPointer)		( const char* pszVarName );
	...
	*	@return WON ID, or -1.
	*/
	unsigned int (*pfnGetPlayerWONId)		( edict_t* pEdict );
	...
	*/
	void		(*pfnInfo_RemoveKey)		( char* pszInfoBuffer, const char* pszKey );

	/**
	*	Gets the given physics keyvalue from the given client's buffer.
	*	@param pClient Client whose buffer will be queried.
	*	@param pszKey Key whose value will be retrieved.
	*	@return The value, or an empty string if the key does not exist.
	*/
	const char *(*pfnGetPhysicsKeyValue)	( const edict_t* pClient, const char* pszKey );

	/**
	*	Sets the given physics keyvalue in the given client's buffer.
	*	@param pClient Client whose buffer will be modified.
	*	@param pszKey Key whose value will be set.
	*	@param pszValue Value to set.
	*/
	void		(*pfnSetPhysicsKeyValue)	( const edict_t* pClient, const char* pszKey, const char* pszValue );
	...
	*/
	unsigned short (*pfnPrecacheEvent)		( int type, const char* pszEventName );
	...
	*	@param pEntity Entity to check.
	*	@param pVisibleSet Buffer detailing the current visible set.
	*	@return Whether the given entity is visible in the given visible set.
	*/
	int			(*pfnCheckVisibility )		( const edict_t* pEntity, unsigned char* pVisibleSet );
	...
	*/
	void		(*pfnDeltaSetField)			( delta_t* pFields, const char* pszFieldName );
	...
	*	@param pszFieldName Field name.
	*/
	void		(*pfnDeltaUnsetField)		( delta_t* pFields, const char* pszFieldName );
	...
	*/
	int			(*pfnCanSkipPlayer)			( const edict_t* pPlayer );

	/**
	*	Finds the index of a delta field.
	*	@param pFields List of fields.
	*	@param pszFieldName Name of the field to find.
	*	@return Index of the delta field, or -1 if the field couldn't be found.
	*/
	int			(*pfnDeltaFindField)		( delta_t* pFields, const char* pszFieldName );
	...
	*	@param piPing Current ping.
	*	@param piPacket_loss Current packet loss, measured in percentage.
	*/
	void		(*pfnGetPlayerStats)		( const edict_t* pClient, int* piPing, int* piPacket_loss );
	...
	*/
	const char* (*pfnGetPlayerAuthId)		( const edict_t* pEdict );
...
//	const char *(*pfnKeyNameForBinding)		( const char* pBinding );
	...
	*	@param pszFileName Name of the file whose size is to be queried
	*	@return File size, or -1 if the file doesn't exist.
	*/
	int			(*pfnGetFileSize)			( const char* pszFileName );

	/**
	*	Gets the average wave length in seconds.
	*	@param pszFilePath Name of the sound.
	*	@return Length of the sound file, in seconds.
	*/
	unsigned int (*pfnGetApproxWavePlayLen) ( const char* pszFilePath );
	...
	*/
	...
	/**
	*	BGC
	*	@param pszLabel Label
	*	@return Number of characters of the localized string referenced by using "pszLabel".
	*/
	int			(*pfnGetLocalizedStringLength)( const char* pszLabel );
	...
	*/
	void (*pfnRegisterTutorMessageShown)( int iMessageID );
	...
	*/
	int (*pfnGetTimesTutorMessageShown)( int iMessageID );
	...
	*/
	void (*ProcessTutorMessageDecayBuffer)( int* pBuffer, int bufferLength );
	...
	*	@param bufferLength Size of the buffer, in bytes.
	*/
	void (*ConstructTutorMessageDecayBuffer)( int* pBuffer, int bufferLength );
	...
	*	@param pszCvarName Cvar to query.
	*/
	void (*pfnQueryClientCvarValue)( const edict_t* pPlayer, const char* pszCvarName );
	...
	*/
	void (*pfnQueryClientCvarValue2)( const edict_t* pPlayer, const char* pszCvarName, int requestID );

	/**
	*	Checks if a command line parameter was provided.
	*	@param pszCmdLineToken Command key to look for.
...
struct DLL_FUNCTIONS
{
	...
	*/
	void			(*pfnSaveGlobalState)		( SAVERESTOREDATA* pSaveData );

	/**
	*	Called by the engine to restore global state.
	*/
	void			(*pfnRestoreGlobalState)	( SAVERESTOREDATA* pSaveData );
	...
	*	@param pState state Entity state data for the entity that is being added.
	*	@param entIndex Index of the entity being considered for addition.
	*	@param pEnt Entity being considered for addition.
	*	@param pHost Client currently being processed.
	...
	*	@param pPlayer Player to retrieve weapon info from.
	...
	*	@param pPlayer Player.
	*/
	void			(*pfnCmdEnd)			( const edict_t *pPlayer );
	...
	*	@param pszArgs Arguments provided by the sender.
	...
	*/
	int				(*pfnInconsistentFile)( const edict_t* player, const char* pszFileName, char* disconnect_message );
