	...
	case CONTROL_QUOTE:
		{
			...
			if( IsValidReadPosition() )
			{
				...
				if( *m_pszCurrentPosition != CONTROL_QUOTE && m_Settings.fLogWarnings )
					Alert( at_error, "CKeyvaluesLexer::ReadNext: unclosed quote!\n" );
	...
	if( result == ReadResult::READ_TOKEN )
	{
		...
		if( !fWasQuoted )
		{
			if( strncmp( pszBegin, "{", 1 ) == 0 )
			{
				...
				if( m_TokenType != TokenType::KEY && !m_Settings.fAllowUnnamedBlocks )
				{
					if( m_Settings.fLogErrors )
						Alert( at_error, "CKeyvaluesLexer::ReadNextToken: illegal block open '%c'!\n", CONTROL_BLOCK_OPEN );
			...
			else if( strncmp( pszBegin, "}", 1 ) == 0 )
			{
				...
				{
					if( m_Settings.fLogErrors )
						Alert( at_error, "CKeyvaluesLexer::ReadNextToken: illegal block close '%c'!\n", CONTROL_BLOCK_CLOSE );
			...
			for( size_t uiIndex = 0; uiIndex < uiMaxSize; )
			{
				if( m_pEscapeSeqConversion->GetDelimiterChar() == pszBegin[ uiIndex ] )
				{
					if( uiIndex + 1 < uiMaxSize )
					{
						...
						{
							if( m_Settings.fLogErrors )
								Alert( at_error, "CKeyvaluesLexer::ReadNextToken: illegal escape sequence '%c%c'!\n", pszBegin[ uiIndex ], pszBegin[ uiIndex + 1 ] );
					...
					{
						if( m_Settings.fLogErrors )
							Alert( at_error, "CKeyvaluesLexer::ReadNextToken: escape sequence delimiter '%c' at the end of a token!\n", pszBegin[ uiIndex - 1 ] );
