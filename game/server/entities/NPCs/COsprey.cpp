...
void COsprey::Flight( )
{
	...
	if (m_iSoundState == 0)
	{
		EMIT_SOUND_DYN( this, CHAN_STATIC, "apache/ap_rotor4.wav", 1.0, 0.15, 0, 110 );
	...
	else
	{
		...
		if (pPlayer)
		{
			...
			{
				m_iPitch = pitch;
				EMIT_SOUND_DYN( this, CHAN_STATIC, "apache/ap_rotor4.wav", 1.0, 0.15, SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
				// ALERT( at_console, "%.0f\n", pitch );
			}
		}
