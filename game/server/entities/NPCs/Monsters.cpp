...
Relationship CBaseMonster::IRelationship( CBaseEntity *pTarget )
{
	...
	return EntityClassifications().GetRelationshipBetween( Classify(), pTarget->Classify() );
}
...
void CBaseMonster :: HandleAnimEvent( AnimEvent_t& event )
{
	...
	case SCRIPT_EVENT_SENTENCE:			// Play a named sentence group
		g_Sentences.PlayRndSz( this, event.options, 1.0, ATTN_IDLE, 0, 100 );
		break;
	...
	case MONSTER_EVENT_BODYDROP_LIGHT:
		if ( pev->flags & FL_ONGROUND )
		{
			if ( RANDOM_LONG( 0, 1 ) == 0 )
			{
				EMIT_SOUND( this, CHAN_BODY, "common/bodydrop3.wav", 1, ATTN_NORM );
			}
			else
			{
				EMIT_SOUND( this, CHAN_BODY, "common/bodydrop4.wav", 1, ATTN_NORM );
			}
		}
		break;
	...
	case MONSTER_EVENT_SWISHSOUND:
		{
			...
			EMIT_SOUND( this, CHAN_BODY, "zombie/claw_miss2.wav", 1, ATTN_NORM );
			break;
		}
...
void CBaseMonster::PlaySentence( const char *pszSentence, float duration, float volume, float attenuation )
{
	if ( pszSentence && IsAlive() )
	{
		if ( pszSentence[0] == '!' )
			...
			g_Sentences.PlayRndSz( this, pszSentence, volume, attenuation, 0, PITCH_NORM );
	}
}
...
{
	EMIT_SOUND( this, CHAN_VOICE, "common/null.wav", 1.0, ATTN_IDLE );
}
