...
void CVoiceStatus::UpdateSpeakerStatus( int entindex, qboolean bTalking )
{
	...
	{
		char msg[256];
		V_sprintf_safe( msg, "CVoiceStatus::UpdateSpeakerStatus: ent %d talking = %d\n", entindex, bTalking );
			...
			if ( !pLabel )
			{
				...
				if ( ( entindex != iLocalPlayerIndex ) || ( pVoiceLoopback && pVoiceLoopback->value ) )
				{
					if( ( pLabel = GetFreeVoiceLabel() ) )
					{
						...
						char paddedName[512];
						V_sprintf_safe( paddedName, "%s   ", info.name );
...
void CVoiceStatus::UpdateServerState(bool bForce)
{
	...
	if(bForce || m_bServerModEnable != bCVarModEnable)
	{
		...
		char str[256];
		V_sprintf_safe(str, "VModEnable %d", m_bServerModEnable);
		ServerCmd(str);
...
void CVoiceStatus::UpdateBanButton(int iClient)
{
	...
	char playerID[ PLAYERID_BUFFER_SIZE ];
	if(!UTIL_GetPlayerUniqueID(iClient+1, playerID))
		return;
